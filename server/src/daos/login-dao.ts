...
export function validateLogin(credentials:any): Promise<Role>{
    const sql = `select ers_user_roles.user_role, ers_users_id as user_id from ers_user_roles join ers_users on ers_users.ers_users_id = ers_user_roles.ers_user_role_id 
